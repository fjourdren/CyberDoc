import { Controller, Post, UseGuards, Req, Res, HttpCode } from '@nestjs/common';
...
export class AuthController {
  ...
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        "username": { type: "string", description: "Username", example: "user@example.com" },
        "password": { type: "string", description: "Password", example: "eV66scN@t5tGG%ND" }
      },
      required: ["username", "password"]
    }
  })
  @ApiOperation({ summary: "Login", description: "Login and store JWT token in a cookie" })
