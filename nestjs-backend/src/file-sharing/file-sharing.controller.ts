...
export class FileSharingController {
    ...
    @ApiOperation({ summary: "Get all files shared with the user", description: "Get all files shared with the user" })
    ...
    @ApiOperation({ summary: "See for a file which users have sharing access to it", description: "See for a file which users have sharing access to it" })
    ...
    @ApiParam({ name: "fileID", description: "File ID", example: "f3f36d40-4785-198f-e4a6-2cef906c2aeb" })
    @ApiParam({ name: "email", description: "User email", example: "email@example.com" })
    @ApiOperation({ summary: "Add sharing access for a user and a file", description: "Add sharing access for a user and a file" })
    @ApiCreatedResponse({ description: "Success", type: GenericResponse })
    @ApiBadRequestResponse({ description: "Cannot share a file with himself OR The email already have access to the file", type: GenericResponse })
    ...
    @ApiParam({ name: "fileID", description: "File ID", example: "f3f36d40-4785-198f-e4a6-2cef906c2aeb" })
    @ApiParam({ name: "email", description: "User email", example: "email@example.com" })
    @ApiOperation({ summary: "Remove sharing access for a user and a file", description: "Remove sharing access for a user and a file" })
    @ApiOkResponse({ description: "Success", type: GenericResponse })
    @ApiBadRequestResponse({ description: "Cannot share a file with himself OR The email don't have access to the file", type: GenericResponse })
