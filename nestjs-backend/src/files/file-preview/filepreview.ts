...
export async function generatePreview(inputPath: string, fileType: "other" | "video" | "image" | "pdf", outputPath: string, options: GeneratePreviewOptions) {

    // Check for supported outputPath format
    await ensureFileExists(inputPath);
    var extOutput = extname(outputPath).toLowerCase().replace('.', '');
    ...
    switch (fileType) {
        ...
        case "other": {
            ...
            var tempPDF = join(tmpdir(), hash + '.pdf');
