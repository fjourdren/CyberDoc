...
export class FileInResponse {
    @ApiProperty({ description: "File ID", example: "f3f36d40-4785-198f-e4a6-2cef906c2aeb" })
    _id: string;
    ...
    @ApiProperty({ description: "Email of users who have signed this file", type: [String], example: ["email@example.com"] })
    signs: string[];

    @ApiProperty({ description: "File share mode", example: ShareMode.READONLY, enum: [ShareMode.READONLY, ShareMode.READWRITE] })
    shareMode: ShareMode;
}
...
export class DirectoryProperties extends FileInResponse {
    @ApiProperty({ description: "Folder content (null for a file)", type: FileInResponse })
    directoryContent: FileInResponse;

    @ApiProperty({ description: "Folder breadcrumb path (null for a file)", type: [FolderBreadcrumbPathItem] })
