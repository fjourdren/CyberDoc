...
export class CryptoService {
  ...
  async setUserPrivateKey(user: User, userHash: string, privateKey: string) {
    user.userKeys.encrypted_private_key = this.aes.encrypt(
      userHash,
      privateKey,
    );
  }
  ...
  getFileAESKey(
    ...
    const encryptedAESKey = user.filesKeys.find(
  ...
  async addFileAESKeyToUser(user: User, fileID: string, fileAESKey: string) {
    const encryptedAESKey = this.rsa.encrypt(
      user.userKeys.public_key,
      fileAESKey,
    );
  ...
  async createAndGetFileAESKey(
    user: User,
    userHash: string,
    fileID: string,
  ): Promise<string> {
    if (this.getFileAESKey(user, userHash, fileID))
      throw new InternalServerErrorException('File AES key already exists');
  ...
  async deleteAllAESKeysForFile(fileID: string): Promise<void> {
    await this.userModel
      .updateMany({}, { $pull: { filesKeys: { file_id: fileID } } })
      .exec();
  }
}
