    ...
    public static async settings(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if(res.locals.APP_JWT_TOKEN.email) {
                ...
                const output: Record<string, IUser | string> = requireNonNull(await UserService.updateProfile(user_hash, new_user_hash, user_id, firstname, lastname, email, password, phoneNumber, secret, twoFactorApp, twoFactorSms, twoFactorEmail));
                res.status(HttpCodes.OK);
                res.json({
                    success: true,
                    msg: "Profile updated",
