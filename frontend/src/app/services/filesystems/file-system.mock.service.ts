...
export class MockFileSystemService {
  ...
  createDirectory(
    ...
    return of(null)
      ...
      .pipe(
        map(() => {
          const newEntry: InternalFileElement = {
  ...
  startFileUpload(file: File, destination: CloudDirectory): void {
    ...
    this._uploadInternal(file.name, file.size).then((success) => {
      if (success) {
        this.filesMap.set(newEntry.id, newEntry);
        this._currentUpload$.emit(null);
      ...
      .pipe(
        map(() => {
          ...
          nodes = nodes.filter((node) =>
            node.name.startsWith(searchParams.name),
          );

          if (searchParams.type !== FileType.Unknown) {
            nodes = nodes.filter(
              (node) =>
                this.fileUtils.getFileTypeForMimetype(node.mimetype) ===
          ...
          nodes = nodes.filter((node) => {
            for (const tagID of searchParams.tagIDs) {
              if (node.tags.map((tag) => tag._id).indexOf(tagID) === -1)
                return false;
            }
            return true;
          });
          ...
          directory.directoryContent = nodes.map((val) =>
            this._convertInternalNodeToCloudNode(val),
          );
          return directory;
        }),
      );
    ...
    return of(null)
      ...
      .pipe(
        map(() => {
          ...
          return this._convertInternalNodeToCloudNode(
            this.filesMap.get(nodeID),
          );
        }),
      );
  }
  ...
  removeTag(node: CloudNode, tag: FileTag): Observable<void> {
    return of(null)
      ...
      .pipe(
        map(() => {
          const internalNode = this.filesMap.get(node._id);
          internalNode.tags = internalNode.tags.filter(
            (val) => val._id !== tag._id,
          );
          this.filesMap.set(internalNode.id, internalNode);
          this._save();
  ...
  share(fileID: string, email: string): Observable<void> {
    ...
    const Respond: RespondShare = { email: '', name: '' };
    let test = false;
    return of(null)
      .pipe(delay(DELAY))
      .pipe(
  ...
  private _deleteInternal(fileID: string) {
    ...
    if (file.mimetype === DIRECTORY_MIMETYPE) {
      for (const child of Array.from(this.filesMap.values()).filter(
        (val) => val.parentID === fileID,
      )) {
  ...
  private _convertInternalNodeToCloudNode(
    internalNode: InternalFileElement,
    fillDirectoryContentAndPath = true,
  ): CloudNode {
    if (internalNode.mimetype === DIRECTORY_MIMETYPE) {
      let directoryContent: CloudNode[] = [];
      let path: PathItem[] = [];
      if (fillDirectoryContentAndPath) {
        directoryContent = Array.from(this.filesMap.values())
          .filter((val) => val.parentID === internalNode.id)
          .map((val) => {
        ...
        while (current) {
          ...
          if (current.parentID) {
            current = Array.from(this.filesMap.values()).filter(
              (val) => val.id === current.parentID,
            )[0];
          } else {
            current = null;
          }
  ...
  private _save() {
    localStorage.setItem(
      '__filesMap',
      JSON.stringify(Array.from(this.filesMap.entries())),
    );
    this._refreshNeeded$.emit(null);
  }
  ...
  private _load() {
    ...
    if (!this.filesMap || this.filesMap.size === 0) {
      ...
      this.filesMap.set('root.sub1.sub', {
        parentID: 'root.sub1',
        name: 'Bills',
        mimetype: DIRECTORY_MIMETYPE,
        id: 'root.sub1.sub',
      ...
      this.filesMap.set('root.sub1.f1', {
        parentID: 'root.sub1',
        name: 'mydoc01.pdf',
      ...
      this.filesMap.set('root.sub1.f2', {
        parentID: 'root.sub1',
        name: 'mydoc02.pdf',
        mimetype: 'application/pdf',
        size: 964,
      ...
      this.filesMap.set('root.sub1.f3', {
        parentID: 'root.sub1',
        name: 'mydoc03.pdf',
        mimetype: 'application/pdf',
        size: 444,
      ...
      this.filesMap.set('root.sub1.sub.f1', {
        parentID: 'root.sub1.sub',
        name: 'bill01.pdf',
        mimetype: 'application/pdf',
        size: 368,
        date: new Date(),
      ...
      this.filesMap.set('root.sub1.sub.f2', {
        parentID: 'root.sub1.sub',
        name: 'bill02.pdf',
        mimetype: 'application/pdf',
        size: 216,
      ...
      this.filesMap.set('root.sub1.sub.f3', {
        parentID: 'root.sub1.sub',
        name: 'bill03.pdf',
        mimetype: 'application/pdf',
        size: 698,
